/**
  ******************************************************************************
  * @file           : led.h
  * @brief          : The header file of TFTLCD device driver
  * @author         : Joe-joe
  * @date           : 2020/2/25
  ******************************************************************************
  * @attention
...
typedef struct
{
    ...
    uint16_t id;                //LCD ID 
    uint8_t dir;                //横竖屏控制：0——竖屏；1——横屏
    uint16_t wramcmd;           //开始写 graphics RAM 指令
    uint16_t setxcmd;           //设置X坐标轴
    uint16_t setycmd;           //设置Y坐标轴
}_lcd_dev;
...
extern uint32_t BACK_COLOR;//背景色，默认白色

//-----------------MCU屏 LCD端口定义----------------//

//#define LCD_LED PBout(15) HAL库在配置时已经进行配置
